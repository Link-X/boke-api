"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
// 这块地方不想转成ts，突然感觉加ts的意义是什么，感觉不怎么会用
const typeOf_js_1 = require("./typeOf.js");
class Verify {
    constructor(data, rules) {
        this.data = null;
        this.rules = {};
        this.$init(data, rules);
    }
    $init(data, rules) {
        // 开发模式
        // if (!data || !objectLen(rules)) {
        //     console.error('设置的校验数据或规则错误')
        //     return
        // }
        this.data = data;
        this.rules = rules;
    }
    ruleCallBack(status) {
        // 自定义校验
        return (cb) => {
            status.status = !!(cb && cb.message);
            status.message = cb && cb.message;
        };
    }
    verifyTop(obj, val) {
        // 校验第一个规则
        const type = (obj.type ? obj.type : typeOf_js_1.getType(val)).toLocaleLowerCase();
        const func = typeOf_js_1.typeOfS[type] || (() => { });
        return !(val && func(val));
    }
    verifyBottom(obj, val) {
        // 校验第二个规则
        const section = obj.min && obj.max && obj.type !== 'date';
        if (!section)
            return false;
        const type = typeOf_js_1.getType(val);
        const len = typeOf_js_1.getTypeLen[type](val);
        const lenSection = (len >= obj.min && len <= obj.max);
        return !lenSection;
    }
    iterator(rules) {
        if (!typeOf_js_1.isObject(rules)) {
            return;
        }
        let status = {
            status: false,
            message: '',
            key: ''
        };
        for (let v of Object.keys(rules)) {
            const judge = Object.assign(Object.assign({}, this.rules[v][0]), this.rules[v][1]);
            const val = this.data[v];
            if (typeOf_js_1.isFunc(judge.validator)) {
                judge.validator(val, this.ruleCallBack(status));
            }
            else if (judge.required) {
                status.status = this.verifyTop(judge, val) || this.verifyBottom(judge, val);
            }
            else if (!judge.required && judge.min && judge.max) {
                status.status = val && this.verifyBottom(judge, val);
            }
            if (status.status) {
                // status 为true停止校验
                status.key = v;
                status.message = status.message ? status.message : judge.message;
                return status;
            }
        }
        return status;
    }
    validate(cb) {
        const status = this.iterator(this.rules);
        const data = {
            key: status.key,
            result: status.status || status.message,
            message: status.message
        };
        cb(data);
        return data;
    }
    addRule(rule) {
    }
}
module.exports = Verify;
