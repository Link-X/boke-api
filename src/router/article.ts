...
router.put('/add/article', (req: Request, res: Response, next: NextFunction) => {
    const params: AddArticle = req.body
    ...
    verifyFunc.validate(status => {
        ...
        try {
            ...
            const userData: TokenData = utils.verifyToken(req.headers.token)
...
router.get('/get/article/list', (req: Request, res: Response, next: NextFunction) => {
    const params: Page = { ...{ page: 1, pageSize: 10 }, ...req.query }
...
router.post('/del/article', (req: Request, res: Response, next: NextFunction) => {
    const params: DelArticle = req.body
    verifyFunc.$init(params, {
        id: [{
            required: true,
            message: '请选择要删除的文章',
...
router.get('/get/article/major', (req: Request, res: Response, next: NextFunction) => {
...
router.get('/get/article/details', (req: Request, res: Response, next: NextFunction) => {
...
router.post('/endit/article', (req: Request, res: Response, next: NextFunction) => {
    const params: EditArticle = req.body
    verifyFunc.$init(params, {
        id: [{
            required: true,
            message: '请输入文章id',
...
router.get('/seach/article', (req: Request, res: Response, next: NextFunction) => {
    const params: SearchArticle = req.query
    verifyFunc.$init(params, {
        query: [{
            required: true,
            message: '请输入需要搜索的内容',
...
router.get('/tab/tags', (req: Request, res: Response, next: NextFunction) => {
    const params: TabArticle = req.query
    verifyFunc.$init(params, {
        tagId: [{
            required: true,
            message: '请选择搜索的类型',
...
router.post('/upload-image', (req: Request, res: Response, next: NextFunction) => {
    ...
    const dataBuffer: Buffer = new Buffer(base64Data, 'base64')
    const randomStr: string = Math.random().toString().split('.')[1] + Date.now() + '.jpg'
    const pathHref: string = '../www/image/' + randomStr
    fs.writeFile(path.resolve(__dirname, pathHref), dataBuffer, function (err) {
        const ip: string = utils.getIp()
        if (err) {
...
router.post('/add/article-comment', (req: Request, res: Response, next: NextFunction) => {
    const params: AddArticleComment = req.body
    verifyFunc.$init(params, {
        text: [{
            required: true,
